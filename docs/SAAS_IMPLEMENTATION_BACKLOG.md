...
5. Add invite and membership management flow. Initial local invite schema, invite acceptance and revoke services, audit events, tenant-guarded invite listing/creation/revoke routes, and settings-page invite management UI now exist in `backend/migrations/004_invites.sql`, `backend/invitations.py`, `backend/api.py`, and `frontend/app/settings/page.tsx`.
